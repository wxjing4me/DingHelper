...
class ProfilesWorker(QObject):

    _finished = pyqtSignal()
    _signal = pyqtSignal(str)
    
    def __init__(self, infoPath='', excelsDir='', profilesDir=''):
        super().__init__()
        self.infoPath = infoPath
        self.excelsDir = excelsDir
    ...
    def work(self):
        ...
        self.generateEveryoneProfile(self.profilesDir, stuInfos, dates, stuDatas)
    ...
    def getStuInfo(self, infoFile):
        self._signal.emit(f'>> 处理学生信息文件: {infoFile}')

        stuInfos = {}

        excel = xlrd_open_workbook(infoFile)
    ...
    def getDatasFromDing(self, dingDir, stuInfos):

        datas = {}
        dates = []
        if not os_path_exists(dingDir):
            print(f'路径不存在：{dingDir}')
            return dates, datas

        excel_list = []
        for filename in os.listdir(dingDir):
            excel_list.append(os_path_join(dingDir, filename))
        ...
        for sno in stuInfos.keys():
            if sno not in datas.keys():
                self._signal.emit(f"缺少人员: {sno} {stuInfos[sno]['sname']}")
                datas[sno] = {}
    ...
    def generateEveryoneProfile(self, saveDir, stuInfos, dates, stuDatas):
        cnt = 0
        total = len(stuInfos.keys())
        for sno, sinfo in stuInfos.items():
            cnt += 1
            self._signal.emit(f">> {cnt}/{total} {sno} {sinfo['sname']}")
            stuResFile = os_path_join(saveDir, f"{sno}-{sinfo['sname']}.xlsx")

            wexcel = xlsxwriter_Workbook(stuResFile)
