from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QTextBrowser, QFileDialog
...
class MainWindows(QWidget):
    ...
    def initUI(self):
        ...
        self.btn_openMapDir = QPushButton('打开地图路径')
        self.btn_openMapDir.clicked.connect(self.clickBtn_openMapDir)
        self.btn_openMapDir.setEnabled(False)
        layout_buttons.addWidget(self.btn_openMapDir)
    ...
    def clickBtn_drawMap(self):
        ...
        if self.MapsDir != '':
            ...
            self.drawMap()
            self.btn_startAnalyse.setEnabled(True)
            self.btn_openMapDir.setEnabled(True)

    def clickBtn_openMapDir(self):
        print('打开地图文件夹：%s' % self.MapsDir)
    ...
    def drawMap(self):
        ...
        self.drawMapThread.finished.connect(lambda: self.updateOutput('生成地图结束！点击【打开地图路径】查看~', True))
